"""Chat-companion — o backend do tutor do livro.
...
_exercicios = Exercicios(config.REPO_ROOT, config.EXERCICIOS_PATH)
_tools = Tools(_index, _store)
...
# A persona do tutor. Esta é a decisão de projeto mais consequente do companion:
...
def _system_prompt(chapter: Optional[int], mode: str, achados: list[dict],
    ...
    return (
        PERSONA + f"\n\nCapacidades ativas agora: {lista}. {modo_txt}{obj}{ctx}"
    )
...
class EntidadeSelecionada(BaseModel):
    type: str
    id: str
    label: str = ""
...
class Snapshot(BaseModel):
    screen: Tela
    selected_entity: Optional[EntidadeSelecionada] = None
    captured_at: str = ""
...
def get_progresso(session_id: str) -> dict:
    """O traço das tentativas — o que a página usa para o selo do exercício."""
    return {"session_id": session_id, "tentativas": _store.progresso(session_id)}


@app.get("/exercicios")
def get_exercicios(capitulo: int) -> dict:
    """Exercícios de um capítulo, SEM os critérios de avaliação (rubrica é do
    servidor: o leitor não deve poder ler por quais critérios será avaliado)."""
    return {"capitulo": capitulo,
            "exercicios": [{"id": e["id"], "titulo": e.get("titulo", ""),
                            "enunciado": e.get("enunciado", ""),
                            "variante": e.get("variante", ""), "tipo": e.get("tipo", "")}
                           for e in _exercicios.do_capitulo(capitulo)]}
...
def _foco_exercicio(inp: ChatIn) -> Optional[dict]:
    """Resolve o exercício declarado pela página. O cliente manda só o id; o
    enunciado e os critérios saem do registro do servidor — nunca do cliente."""
    snap = inp.snapshot
    ent = snap.selected_entity if snap else None
    if not ent or ent.type != "exercicio":
        return None
    ex = _exercicios.get(ent.id)
    if ex is None:
        raise HTTPException(status_code=400, detail=f"exercício desconhecido: {ent.id}")
    return ex
...
def _preparar_chat(inp: ChatIn, request: Request) -> tuple[str, str, list, set]:
    ...
    foco = _foco_exercicio(inp)

    # Camadas SEPARADAS e rotuladas (spec 005): persona, exercício em foco,
    # contexto de tela e trechos do livro entram como mensagens de sistema
    # distintas — nunca concatenadas ao texto do leitor. Só assim dá para
    # auditar depois qual contexto originou qual resposta.
    history = [{"role": "system", "content": _system_prompt(inp.chapter, mode, achados, goal)}]
    if foco:
        history.append({"role": "system", "content": _exercicios.para_prompt(foco)})
    if inp.snapshot:
        history.append({"role": "system", "content":
                        "Contexto de tela declarado pela página: "
                        f"{inp.snapshot.screen.title or inp.snapshot.screen.id}"})
    history += _store.history(inp.session_id, limit=40)

    # O contexto do turno vai para as tools. A sessão e o exercício em foco vêm
    # daqui, nunca dos argumentos do modelo.
    _tools.contexto = {"session_id": inp.session_id,
                       "exercicio": {"id": foco["id"], "capitulo": foco.get("capitulo")} if foco else None}

    ativas = tools_ativas(inp.chapter, mode)
    if not foco:
        ativas = ativas - {"exercicio_registrar_tentativa"}  # sem foco, a ação não existe
    return mode, byok, history, ativas, achados
