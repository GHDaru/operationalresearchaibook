...
class StorePort(Protocol):
    ...
    # ---- exercícios (spec 005) ----
    # O registro da tentativa é, ao mesmo tempo, o dado de domínio (o progresso
    # do leitor) e o traço de execução da ação governada. Neste produto os dois
    # coincidem — não há por que manter duas tabelas.
    def registrar_tentativa(self, session_id: str, exercicio_id: str, capitulo: int,
                            veredito: str, feedback: str,
                            criterios: list[str]) -> dict: ...
    def progresso(self, session_id: str) -> list[dict]: ...
...
class MemoryStore:
    ...
    def delete_session(self, session_id: str) -> None:
        ...
        self._tent.pop(session_id, None)   # direito ao esquecimento vale para o progresso
    ...
    # ---- exercícios (spec 005) ----
    def registrar_tentativa(self, session_id: str, exercicio_id: str, capitulo: int,
                            veredito: str, feedback: str, criterios: list[str]) -> dict:
        lista = self._tent.setdefault(session_id, [])
        # Dedup pela chave natural: o mesmo veredito para o mesmo exercício não
        # vira dois registros (clique duplo, retry de rede, modelo insistente).
        for t in lista:
            if t["exercicio_id"] == exercicio_id and t["veredito"] == veredito:
                return dict(t, duplicada=True)
        reg = {"session_id": session_id, "exercicio_id": exercicio_id,
               "capitulo": capitulo, "veredito": veredito, "feedback": feedback,
               "criterios": list(criterios), "ts": time.time(),
               "tentativa_n": len(lista) + 1}
        lista.append(reg)
        return dict(reg, duplicada=False)

    def progresso(self, session_id: str) -> list[dict]:
        return list(self._tent.get(session_id, []))
...
class PostgresStore:
    ...
    def _init_schema(self) -> None:
        with self._conn() as conn, conn.cursor() as cur:
            cur.execute("""
                ...
                -- Tentativas de exercício (spec 005). ON DELETE CASCADE faz o
                -- direito ao esquecimento valer também para o progresso.
                ...
                -- Chave natural: dedup de clique duplo sem inventar protocolo.
                CREATE UNIQUE INDEX IF NOT EXISTS uq_tentativa
                    ON tentativas(session_id, exercicio_id, veredito);
                CREATE INDEX IF NOT EXISTS idx_tentativa_sessao
                    ON tentativas(session_id, id);
            """)
            conn.commit()
    ...
    def registrar_tentativa(self, session_id: str, exercicio_id: str, capitulo: int,
        ...
        with self._conn() as conn, conn.cursor() as cur:
            cur.execute(
                "INSERT INTO tentativas(session_id, exercicio_id, capitulo, veredito, feedback, criterios) "
                "VALUES (%s, %s, %s, %s, %s, %s) "
                "ON CONFLICT (session_id, exercicio_id, veredito) DO NOTHING RETURNING id",
                (session_id, exercicio_id, capitulo, veredito, feedback, _json.dumps(criterios)))
            inserida = cur.fetchone() is not None
            cur.execute("SELECT count(*) FROM tentativas WHERE session_id = %s AND exercicio_id = %s",
                        (session_id, exercicio_id))
            n = cur.fetchone()[0]
            conn.commit()
        return {"session_id": session_id, "exercicio_id": exercicio_id, "capitulo": capitulo,
                "veredito": veredito, "feedback": feedback, "criterios": list(criterios),
                "tentativa_n": n, "duplicada": not inserida}

    def progresso(self, session_id: str) -> list[dict]:
        with self._conn() as conn, conn.cursor() as cur:
            cur.execute("SELECT exercicio_id, capitulo, veredito, feedback, criterios, "
                        "extract(epoch from created_at) FROM tentativas "
                        "WHERE session_id = %s ORDER BY id", (session_id,))
            linhas = cur.fetchall()
        return [{"exercicio_id": r[0], "capitulo": r[1], "veredito": r[2], "feedback": r[3],
                 "criterios": r[4] or [], "ts": r[5], "tentativa_n": i + 1}
