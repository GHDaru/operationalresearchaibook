"""ToolPort — o registro de tools SEGURAS do companion.

Regra do cap. 07 aplicada à superfície pública: nada de shell, nada de leitura
arbitrária de disco, nada de rede de saída. Só tools sandbox, com efeito
limitado e previsível. Cada tool tem um schema (dialeto OpenAI) e está amarrada
a uma capacidade em `capabilities.py` — o gating decide quais entram no loop.

Nota didática: derivar schema de tipos é a lição da etapa 02 (cap. 05); aqui os
schemas ainda são à mão, de propósito — a mesma dor que justifica aquela etapa.
"""

from __future__ import annotations

import ast
import datetime
import operator
from typing import Callable

from ragindex import BookIndex


# ------------------------------------------------------ implementações seguras

def _hora(_args: dict) -> str:
    return datetime.datetime.now().isoformat(timespec="seconds")


# Avaliador aritmético seguro: só números e + - * / ** ( ) unário. Sem nomes,
# sem chamadas, sem atributos — não é eval() do Python (que seria uma ferida).
_OPS = {
    ast.Add: operator.add, ast.Sub: operator.sub, ast.Mult: operator.mul,
    ast.Div: operator.truediv, ast.Pow: operator.pow, ast.Mod: operator.mod,
    ast.USub: operator.neg, ast.UAdd: operator.pos,
}


def _eval_no(no: ast.AST) -> float:
    if isinstance(no, ast.Constant) and isinstance(no.value, (int, float)):
        return no.value
    if isinstance(no, ast.BinOp) and type(no.op) in _OPS:
        return _OPS[type(no.op)](_eval_no(no.left), _eval_no(no.right))
    if isinstance(no, ast.UnaryOp) and type(no.op) in _OPS:
        return _OPS[type(no.op)](_eval_no(no.operand))
    raise ValueError("expressão não permitida")


def _calcular(args: dict) -> str:
    expr = str(args.get("expressao", ""))
    try:
        return str(_eval_no(ast.parse(expr, mode="eval").body))
    except Exception as exc:  # erro volta como texto para o modelo decidir
        return f"erro: {exc}"


class Tools:
    """Fecha as tools sobre o índice do livro (dependência injetada).

    `contexto` é o estado do turno — hoje, o exercício em foco e a sessão. Ele é
    injetado pelo app a cada turno, nunca vem do modelo: é o que impede o modelo
    de registrar tentativa para um exercício que não é o da página.
    """

    def __init__(self, index: BookIndex, store=None) -> None:
        self._index = index
        self._store = store
        self.contexto: dict = {}
        self.impls: dict[str, Callable[[dict], str]] = {
            "hora": _hora,
            "calcular": _calcular,
            "buscar_no_livro": self._buscar_no_livro,
            "exercicio_registrar_tentativa": self._registrar_tentativa,
        }

    def _registrar_tentativa(self, args: dict) -> str:
        """Ação governada de risco `write`: reversível, executa direto e deixa traço.

        Duas guardas, ambas fora do alcance do modelo:
          - a sessão vem do contexto do turno, nunca dos argumentos;
          - o exercício precisa ser o que a PÁGINA declarou. Se o modelo tentar
            registrar outro, a ação é recusada — é o substituto barato do
            `context_hash` do padrão APH: comparar o identificador entrega quase
            todo o valor por uma comparação de string.
        """
        ctx = self.contexto or {}
        sid, foco = ctx.get("session_id"), ctx.get("exercicio")
        if not sid or not foco:
            return "erro: nenhum exercício em foco nesta página; não há o que registrar."
        pedido = str(args.get("exercicio_id", "")).strip()
        if pedido and pedido != foco["id"]:
            return (f"erro: o exercício em foco é '{foco['id']}', não '{pedido}'. "
                    "Só é possível registrar a tentativa do exercício da página atual.")
        veredito = str(args.get("veredito", "")).strip().lower()
        if veredito not in ("aprovado", "parcial", "refazer"):
            return "erro: veredito deve ser 'aprovado', 'parcial' ou 'refazer'."
        if self._store is None:
            return "erro: persistência indisponível."
        reg = self._store.registrar_tentativa(
            session_id=sid, exercicio_id=foco["id"], capitulo=int(foco.get("capitulo") or 0),
            veredito=veredito, feedback=str(args.get("resumo_feedback", ""))[:600],
            criterios=list(args.get("criterios_atendidos", []) or []))
        if reg.get("duplicada"):
            return f"tentativa já registrada antes com o veredito '{veredito}' — nada foi duplicado."
        return (f"tentativa registrada: exercício {foco['id']}, veredito '{veredito}' "
                f"(tentativa {reg.get('tentativa_n')}).")

    def _buscar_no_livro(self, args: dict) -> str:
        achados = self._index.buscar(str(args.get("consulta", "")), k=4)
        if not achados:
            return "nada encontrado no livro para essa consulta."
        return "\n\n".join(f"[{a['fonte']} · {a['titulo']}] {a['trecho']}" for a in achados)

    def schemas_para(self, nomes: set[str]) -> list[dict]:
        """Só os schemas das tools ativas (gating)."""
        todos = {
            "hora": {"type": "function", "function": {
                "name": "hora",
                "description": "Retorna a data e hora atuais (ISO).",
                "parameters": {"type": "object", "properties": {}, "required": []}}},
            "calcular": {"type": "function", "function": {
                "name": "calcular",
                "description": "Avalia uma expressão aritmética segura (+ - * / ** %).",
                "parameters": {"type": "object",
                               "properties": {"expressao": {"type": "string",
                                              "description": "ex.: (2+3)*4"}},
                               "required": ["expressao"]}}},
            "buscar_no_livro": {"type": "function", "function": {
                "name": "buscar_no_livro",
                "description": "Busca trechos relevantes no texto do livro.",
                "parameters": {"type": "object",
                               "properties": {"consulta": {"type": "string",
                                              "description": "o que procurar no livro"}},
                               "required": ["consulta"]}}},
            "exercicio_registrar_tentativa": {"type": "function", "function": {
                "name": "exercicio_registrar_tentativa",
                "description": (
                    "Registra a tentativa do leitor no exercício EM FOCO nesta página, "
                    "depois de você avaliá-la contra os critérios. Use 'aprovado' quando "
                    "todos os critérios forem atendidos, 'parcial' quando faltar algum, e "
                    "'refazer' quando o essencial não foi atingido. Não use antes de o "
                    "leitor responder."),
                "parameters": {"type": "object",
                               "properties": {
                                   "exercicio_id": {"type": "string",
                                                    "description": "id do exercício em foco"},
                                   "veredito": {"type": "string",
                                                "enum": ["aprovado", "parcial", "refazer"]},
                                   "criterios_atendidos": {"type": "array", "items": {"type": "string"},
                                                           "description": "critérios que a resposta atendeu"},
                                   "resumo_feedback": {"type": "string",
                                                       "description": "uma frase sobre o que faltou ou o que ficou bom"}},
                               "required": ["veredito"]}}},
        }
        return [todos[n] for n in nomes if n in todos]

    def executar(self, nome: str, args: dict, permitidas: set[str]) -> str:
        if nome not in permitidas:                       # gating também na execução
            return f"erro: ferramenta '{nome}' não está habilitada neste capítulo."
        impl = self.impls.get(nome)
        return impl(args) if impl else f"erro: ferramenta desconhecida '{nome}'"
